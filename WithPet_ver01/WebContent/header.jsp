<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<!-- Header -->
<div id="header-wrapper">
	<header id="header" class="container">
		<!-- Logo -->
		<div id="logo">
			<a href="/main/main.jsp"> <img class="d-block mx-auto mb-4"
				src="/logo.png" alt="" width="150" height="75">
			</a>
		</div>

		<!-- Nav -->
		<nav id="nav">
			<ul>
				<li><a href="/main/main.jsp">홈</a></li>
				<li><a href="/hotel/hotel.jsp">호텔 찾기</a>
					<ul>
						<li><a href="/hotel/hotellocation.jsp">지도별 검색</a></li>
						<li><a href="/hotel/hoteltype.jsp">종별 검색</a></li></li>
			</ul>
			</li>
			<li><a href="/petsitter/petsitter.jsp">펫시터 찾기</a></li>
			<li><a href="/product/ShopMallMain.jsp">반려동물 용품</a></li>
			<li><a href="/community/community.jsp">커뮤니티</a>
				<ul>
					<li><a href="/community/community_encyclopedia.jsp">보호자
							대백과</a></li></li>
			</ul>
			</li>
			<li><a href="/hotel/withpetjoin.jsp">위드펫과 함께하기</a>
				<ul>
					<li><a href="/hotel/hotelform.jsp">호텔 등록 신청</a></li>
					<li><a href="/petsitter/petsitterform.jsp">펫시터 등록 신청</a></li>
				</ul></li>
			<li><a href="/user/login.jsp">로그인</a></li>
			<li><a href="/user/signin.jsp">회원가입</a></li>
			<li><a href="/basket/CartView2.jsp">장바구니</a></li>
			</ul>
			</ul>

		</nav>
	</header>
					<hr>
</div>
